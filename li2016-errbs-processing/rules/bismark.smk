...
rule bismark:
    input:
        fastq = [RESULT_DIR / '01_trim-galore' / '{sample}.trimmed.fastq.gz'],
        reference_dir = config['reference']['dir'],
        bisulfite_genome_dir = Path(config['reference']['dir']) / 'Bisulfite_Genome',
    output:
        temp(RESULT_DIR / '02_bismark' / '{sample}' / '{sample}.bismark.bam'),
        RESULT_DIR / '02_bismark' / '{sample}' / '{sample}.bismark_report.txt',
    params:
        extra = BS['extra'],
